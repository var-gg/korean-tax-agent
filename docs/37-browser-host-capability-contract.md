...
- T11 upgrades snapshot-backed inspection/ref context into a first-class artifact (`snapshotContext.artifact`) with explicit `artifactId`, `version`, and `capturedAt`,
- snapshot-backed action requests should echo the exact snapshot artifact/version they were derived from when acting on `aria-ref` locators, and receipts should record the current snapshot context actually used,
- ref freshness should be evaluated against snapshot artifact identity/version rather than adapter-local inspection heuristics,
...
- T15 extends snapshot inspection with typed `inspection.locatorCandidates[]` outputs for snapshot-derived locator/ref material. T16 adds deterministic caller-visible `candidate.guidance` metadata for manual comparison only: stable `ranking` metadata, inspectable `signals`, and optional human-readable `rationale`. T17 adds caller-visible `candidate.staleness` metadata that reports whether a candidate is still tied to the currently bound snapshot artifact/version, has become stale after snapshot turnover, or cannot currently be compared because no bound snapshot artifact/version is available. Those candidates must already carry coherent snapshot identity plus provenance/evidence, and the staleness field is metadata only, so the manual loop remains explicit and narrow: inspect candidates, compare them yourself, notice stale/current annotations, choose one, submit explicit rebinding, then execute the action. The host/core boundary still must not auto-acquire, auto-select, auto-rebind, or retry.
