...
- T15 extends snapshot inspection with typed `inspection.locatorCandidates[]` outputs for snapshot-derived locator/ref material. Those candidates must already carry coherent snapshot identity plus provenance/evidence, so the manual loop is explicit and narrow: inspect candidates, choose one, submit explicit rebinding, then execute the action. The host/core boundary still must not auto-acquire, auto-select, auto-rebind, or retry.
