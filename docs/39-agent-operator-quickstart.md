...
- submission-facing read models should surface `filingWindowState`, `filingWindowHint`, `seasonalityWarningCode`, and `submissionAttemptAllowed` so off-season portal behavior is not misread as a generic auth/UI failure; default lane is May 1-May 31 of the following year, while sincere-filing-confirmation cases may stay open through June 30, both with non-business-day extension
...
- for repeated pilot runs, export a standardized replay bundle; see [41-pilot-replay-bundle.md](./41-pilot-replay-bundle.md)
