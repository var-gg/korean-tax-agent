...
목적:
- narration/chat/operator 확인용으로 사람이 읽기 쉬운 filing summary 반환
...
포맷 메모:
- `operatorUpdate`는 Discord 같은 operator surface에 바로 붙일 수 있도록 compact하게 유지하는 것이 좋습니다.
- review pending / collection blocked / ready for assist / submission in progress 같은 상태별 템플릿을 두는 것이 좋습니다.

설명:
- agent가 현재 상태를 자연어로 설명해야 할 때 우선적으로 쓰기 좋은 narrative read model입니다.
- `short`는 blocker와 next step 중심, `standard`는 draft/readiness 맥락까지 함께 주는 형태가 적절합니다.
- Discord 같은 operator reply에서는 raw field를 다시 조합하기보다 `operatorUpdate`를 우선 사용하는 것이 좋습니다.
- readiness/policy branching에는 `readinessState`를 우선 사용하는 것이 좋습니다.
- 현재 blocker 렌더링, ordered blocker context, submission comparison 상태에는 `runtimeSnapshot`을 우선 사용하는 것이 좋습니다.
- `blockers`는 가벼운 compatibility summary로는 유용하지만, 가장 풍부한 blocker truth로 취급하지 않는 것이 좋습니다.
- `lastBlockingReason` 같은 legacy 단일값 필드는 canonical state가 아니라 compatibility data로 보는 것이 좋습니다.

예시 통합:
- `tax.filing.get_summary` 호출
- Discord/operator surface에서는 `operatorUpdate` 전송
- 일반 chat surface에서는 `headline + summaryText`를 쓰고, 필요하면 active blockers / submission comparison 같은 `runtimeSnapshot` 기반 정보를 덧붙임
- review escalation, submission prep 차단, user intervention 요청 같은 판단에는 `readinessState` 사용
- migration note: 기존 consumer가 `blockers` / `lastBlockingReason`를 보고 있어도 단기 호환은 가능하지만, 새 구현은 `readinessState`로 분기하고 blocker 렌더링은 `runtimeSnapshot` 기준으로 옮기는 것이 좋습니다.
- facade helper: `invokeAndFormatFilingSummaryForDiscord(...)`, `invokeAndFormatFilingSummary(..., 'generic')`
- 샘플: `examples/filing-summary-reply-example.ts`

알림 패턴:
- Core MCP 동작:
  - 이전 filing summary snapshot(`status`, `nextRecommendedAction`, `operatorUpdate`, 그리고 가능하면 `runtimeSnapshot.blockerCodes` / `runtimeSnapshot.activeBlockers`) 보관
  - workflow 진행 후 새 snapshot 재계산
  - status / blocker 상태 / next action이 바뀐 경우에만 알림 전송
  - operator routing을 위해 severity(`high`, `medium`, `info`, `none`)도 함께 분류
  - severity를 `operator-immediate`, `operator-watch`, `operator-updates`, `drop` 같은 추상 route로 매핑
  - route를 dispatch-plan adapter로 host/runtime이 선택한 target에 연결
  - 전송 전 duplicate suppression / cooldown 적용 가능(예시 기본값: high=10분, medium=30분, info=2시간)
- Integration reference pattern(MCP core 필수 아님, host/runtime별 선택 사항):
  - dedupe가 turn 간에도 동작하도록 last delivery record를 store에 저장 가능
  - 프로세스 재시작 이후에도 유지해야 하면 file-backed store 사용 가능
  - 전송 전 `medium` / `info` dispatch plan을 target/route 기준 digest로 묶을 수 있음
  - delivery-policy helper로 `high`는 immediate send, `medium` / `info`는 digest 생성으로 분기 가능
  - sender adapter로 immediate send / digest를 provider-facing payload(`channel`, `target`, `message`)로 변환한 뒤 runtime messenger에 넘길 수 있음
- 데모/테스트에서는 이전 snapshot에서 전환된 snapshot을 직접 시뮬레이션해도 괜찮음
- 샘플:
...
목적:
- 현재 filing workspace snapshot을 상태 조회용 read model로 반환
...
설명:
- agent가 현재 진행상황을 설명하거나 다음 행동을 정할 때 우선적으로 쓰기 좋은 상태 조회 인터페이스입니다.
