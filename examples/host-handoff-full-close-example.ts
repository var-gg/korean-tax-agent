...
const fixturePath = process.argv[2] ?? join(process.cwd(), 'examples', 'acceptance-fixtures', 'full-close-940909-freelancer-two-withholding.json');
...
for (const observation of fixture.observations ?? []) {
  runtime.invoke('tax.sources.record_collection_observation', { workspaceId, sourceId: connect.data.sourceId, ...observation });
}
...
if (reviewItems.length > 0) runtime.invoke('tax.classify.resolve_review_item', { reviewItemIds: reviewItems.map((x: any) => x.reviewItemId), selectedOption: 'keep_draft_value', rationale: 'example grouped review resolution', approverIdentity: 'example:operator' });
...
const approval = runtime.invoke('tax.filing.record_submission_approval', { workspaceId, draftId: draft.data.draftId, approvedBy: 'example:operator' });
const result = runtime.invoke('tax.browser.record_submission_result', { workspaceId, draftId: draft.data.draftId, result: fixture.expected.submissionResult, receiptArtifactRefs: [`receipt_${fixture.scenarioId}`], receiptNumber: `receipt-${fixture.scenarioId}` });
...
console.log(JSON.stringify({ workspaceId, prepareReady: prepare.ok, assistSessionClosed: runtime.getBrowserAssistSession(workspaceId) === undefined, nextAfterAssist: resume.nextRecommendedAction, nextAfterApproval: approval.nextRecommendedAction, result: result.data.submissionResult.result, exportFormats: exported.data.includedFormats }, null, 2));
