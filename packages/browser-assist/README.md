...
- T12 adds narrow host-agnostic `recoveryAdvice` metadata on explicit snapshot-bound action failures so callers can inspect whether they need to reinspect the current target, reacquire a fresh snapshot artifact, or rebind/reacquire the locator from that fresh snapshot. T14 extends that advice with optional expected locator provenance metadata (`snapshot-derived`, derivation basis, snapshot artifact, inspection URL/source) so the next manual acquisition can be audited. T15 adds typed `inspection.locatorCandidates[]` outputs for snapshot-derived locator material, T16 adds deterministic `candidate.guidance` metadata (`ranking`, inspectable `signals`, optional `rationale`) so callers can compare/display/sort candidates themselves, and T17 adds `candidate.staleness` metadata so callers can see whether a candidate is still tied to the currently bound snapshot artifact/version or has become stale after snapshot turnover. That staleness field is inspectable status only: callers still choose whether to discard, de-prioritize, reacquire, or explicitly rebind, and the core does not auto-recover, auto-resnapshot, auto-select a candidate, or retry.
