...
function getMissingRequiredFields(name: SupportedRuntimeToolName, input: Record<string, unknown>): string[] {
  const requiredFieldsByTool: Partial<Record<SupportedRuntimeToolName, string[]>> = {
    ...
    'tax.withholding.list_records': ['workspaceId'],
    'tax.profile.detect_filing_path': ['workspaceId'],
    'tax.profile.upsert_facts': ['workspaceId', 'facts'],
    'tax.profile.list_missing_facts': ['workspaceId'],
    ...
    'tax.filing.list_adjustment_candidates': ['workspaceId'],
    ...
    'tax.filing.record_submission_approval': ['workspaceId', 'draftId', 'approvedBy'],
    'tax.filing.export_package': ['workspaceId', 'formats'],
    'tax.browser.record_submission_result': ['workspaceId', 'draftId', 'result'],
