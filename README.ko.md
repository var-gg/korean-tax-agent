...
최소 real-browser bridge는 `SystemBrowserRuntimeAdapter`로 제공되고, 그 위에 host-agnostic browser-host runtime seam, capability reporting, 그리고 in-repo relay stub 또는 외부 OpenClaw runtime/client path에 실제로 바인딩되는 production-style `OpenClawBrowserToolTransport` 둘 다에 연결될 수 있는 첫 concrete `OpenClawBrowserHostExecutor` adapter가 추가되었습니다.
